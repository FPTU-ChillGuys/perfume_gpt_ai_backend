...
1. **Nhận diện Ý định (Intent Recognition)**:
   - **Search**: Tìm sản phẩm khi người dùng đưa ra các thuộc tính/tiêu chí.
   - **Consult**: Tư vấn, so sánh hoặc hỏi về thông tin chi tiết của sản phẩm.
...
2. **Lọc từ khóa (Keyword Filtering)**:
...
3. **Cấu trúc Logic (CNF - Conjunctive Normal Form)**:
...
3. **Quy trình Tìm kiếm & Chuẩn hóa (Search & Normalization Workflow)**:
...
4. **Ánh xạ Sắp xếp (Sorting Mapping Dictionary)**:
...
2. **normalizationMetadata**: Cập nhật trường \`corrected\` của từng từ khóa bằng nhãn chuẩn (hoặc mảng các nhãn chuẩn) tìm thấy được từ Tool.
...
5. Trả về JSON theo schema \`AnalysisObject\`.
