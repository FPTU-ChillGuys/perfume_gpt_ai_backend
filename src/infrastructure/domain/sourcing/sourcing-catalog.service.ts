...
export class SourcingCatalogService {
  ...
  constructor(
    private readonly redisRequestResponseService: RedisRequestResponseService
  ) {}
   ...
   */
  async getCatalogsAsync(
    variantId: string
  ): Promise<BaseResponseAPI<CatalogItemResponse[]>> {
    try {
      this.logger.log(
        `[Sourcing] Requesting catalogs for variantId=${variantId}`
      );
      ...
      this.logger.error(
        `[Sourcing] Failed to get catalogs for variantId=${variantId}: ${err.message}`
      );
      return {
        success: false,
        error:
          err.message ||
          'Timeout or internal error requesting catalogs via Redis.',
        payload: []
      };
    }
  }
