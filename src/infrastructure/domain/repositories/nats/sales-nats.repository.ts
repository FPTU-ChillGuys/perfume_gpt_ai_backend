...
import { NatsSalesAnalyticsResponse } from 'src/application/dtos/response/nats/nats-sales.response';
...
export class SalesNatsRepository {
   ...
   */
  async getVariantSalesAnalytics(months = 2): Promise<NatsSalesAnalyticsResponse[]> {
    this.logger.log(`[NATS] ${this.i18n.t('sales.get_analytics')}: ${months} months`);
    return await this.natsRpc.sendRequest<NatsSalesAnalyticsResponse[]>(
