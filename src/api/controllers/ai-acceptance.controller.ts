...
export class AIAcceptanceController {
  ...
  @Post('response')
  @ApiOperation({
    summary: 'Tạo AI acceptance pending theo response-level (backend-first)'
  })
  ...
  async createPendingResponseAcceptance(
    ...
    this.logger.log(
      `[POST] /ai-acceptance/response - contextType=${request.contextType}, sourceRefId=${request.sourceRefId}`
    );
    const result =
      await this.aiAcceptanceService.createPendingResponseAcceptance({
    ...
    this.logger.log(
      `[POST] /ai-acceptance/response result: success=${result.success}${result.data ? `, id=${result.data.id}` : ''}`
    );
    return result;
  }
  ...
  async clickAIAcceptance(
    ...
    const result =
      await this.aiAcceptanceService.markAcceptedByAcceptanceId(aiAcceptanceId);
    this.logger.log(
      `[POST] /ai-acceptance/click/${aiAcceptanceId} result: success=${result.success}`
    );
    return result;
  }
  ...
  async updateAIAcceptanceData(
    ...
    this.logger.log(
      `[POST] /ai-acceptance/${id} result: success=${result.success}`
    );
    return result;
  }
  ...
  async getAIAcceptanceRate(
    ...
    this.logger.log(
      `[GET] /ai-acceptance/rate - isAccepted=${isAccepted}, contextType=${contextType}`
    );
  ...
  @Get('metrics')
  @ApiOperation({
    summary: 'Lấy metrics acceptance theo context (accepted/pending/no-click)'
  })
  @ApiQuery({
    name: 'contextType',
    required: false,
