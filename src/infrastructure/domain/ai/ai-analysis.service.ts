...
import {
  ...
  SurveyAnswerAnalysisObject
} from 'src/chatbot/output/survey-question.analysis.output';
...
export class AiAnalysisService {
  ...
  async analyze(
    ...
    try {
      ...
      this.logger.log(
        `[AiAnalysis] Analysis completed. Intent: ${analysis.intent}`
      );
      return analysis;
    } catch (error) {
  ...
  async analyzeIntentOnly(
    currentMessage: string,
    previousMessages?: string
  ): Promise<IntentOnlyObject | null> {
    try {
      ...
      this.logger.log(
        `[AiAnalysis] Intent analysis completed. Intent: ${analysis.intent}`
      );
      return analysis;
    } catch (error) {
  ...
  async analyzeSurvey(
    ...
    try {
      ...
      const analysis = JSON.parse(result) as AnalysisObject;
      this.logger.log(
        `[AiAnalysis] Survey analysis completed. Explanation: ${analysis.explanation}`
      );
      return analysis;
    } catch (error) {
  ...
  async analyzeTrend(
    ...
    try {
      this.logger.log(
        `[AiAnalysis] Starting trend analysis for ${trendSignals.length} signals...`
      );
      ...
      this.logger.log(
        `[AiAnalysis] Trend analysis completed. Logic groups: ${analysis.logic?.length ?? 0}`
      );
      return analysis;
    } catch (error) {
   ...
   */
  async analyzeSurveyAnswer(qa: {
    question: string;
    answer: string;
  }): Promise<AnalysisObject | null> {
    try {
      this.logger.log(
        `[AiAnalysis] Analyzing survey answer: ${qa.answer.substring(0, 50)}...`
      );
      if (!qa || !qa.answer) {
