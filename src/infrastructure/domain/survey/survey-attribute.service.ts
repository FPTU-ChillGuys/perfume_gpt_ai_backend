...
export class SurveyAttributeService {
  ...
  getAvailableAttributeTypes(): SurveyAttributeTypeInfo[] {
    return [
      { type: 'gender', label: 'Giới tính', description: 'Phân loại theo giới tính (Nam, Nữ, Unisex)' },
      { type: 'brand', label: 'Thương hiệu', description: 'Thương hiệu nước hoa (Chanel, Dior, ...)' },
      { type: 'category', label: 'Danh mục', description: 'Danh mục sản phẩm' },
      { type: 'origin', label: 'Xuất xứ', description: 'Quốc gia xuất xứ (Pháp, Ý, ...)' },
      { type: 'concentration', label: 'Nồng độ', description: 'Nồng độ nước hoa (EDP, EDT, Parfum, ...)' },
      { type: 'note', label: 'Nốt hương', description: 'Các nốt hương nước hoa (Vanilla, Rose, ...)' },
      { type: 'family', label: 'Nhóm hương', description: 'Họ nhóm hương (Floral, Woody, ...)' },
      { type: 'attribute', label: 'Thuộc tính sản phẩm', description: 'Thuộc tính mở rộng (Occasion, Season, Age, ...)' },
      { type: 'budget', label: 'Ngân sách', description: 'Khoảng giá mong muốn (từ-đến)' },
    ];
  }

  /** Lấy tất cả giá trị của 1 loại thuộc tính, trả về kèm queryFragment */
  async getAttributeValues(type: SurveyAttributeType): Promise<SurveyAttributeValuesResponse> {
  ...
  private async getGenderValues(): Promise<SurveyAttributeValuesResponse> {
    ...
    const genders = [...new Set(
      products.map(p => p.Gender?.trim()).filter((g): g is string => Boolean(g) && g !== "N'")
    )];
  ...
  private async getOriginValues(): Promise<SurveyAttributeValuesResponse> {
    ...
    const origins = [...new Set(
      products.map(p => p.Origin?.trim()).filter((o): o is string => Boolean(o) && o !== "N'")
    )].sort();
