...
export class Migration20260315153000 extends Migration {
  ...
  override async up(): Promise<void> {
    this.addSql(`create table "event_log" ("id" uuid not null default gen_random_uuid(), "created_at" timestamptz not null, "updated_at" timestamptz not null, "is_active" boolean not null default true, "user_id" text null, "event_type" text check ("event_type" in ('message', 'search', 'quiz')) not null, "entity_type" text check ("entity_type" in ('conversation', 'search', 'quiz')) not null, "entity_id" uuid null, "content_text" text null, "metadata" jsonb null, constraint "event_log_pkey" primary key ("id"));`);
