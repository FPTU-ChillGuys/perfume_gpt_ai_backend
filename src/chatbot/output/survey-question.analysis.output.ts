...
export const surveyAnswerAnalysisSchema = z.object({
    ...
    logic: z.array(z.union([z.string(), z.array(z.string())]))
        .nullable()
        .describe('CNF logic for this specific answer only. Keep focused on ONE intent. MUST use searchMasterData to normalize keywords.'),
    ...
    })).nullable().describe('Log of keyword normalization process via searchMasterData')
}).describe('Analysis result for a single survey answer. MUST use searchMasterData tool to normalize keywords before building logic.');

export type SurveyAnswerAnalysisObject = z.infer<typeof surveyAnswerAnalysisSchema>;
