...
export class Migration20260123183347 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table "survey_question" ("id" uuid not null default gen_random_uuid(), "created_at" timestamptz not null, "updated_at" timestamptz not null, "question" varchar(255) not null, constraint "survey_question_pkey" primary key ("id"));`);

    this.addSql(`create table "survey_answer" ("id" uuid not null default gen_random_uuid(), "created_at" timestamptz not null, "updated_at" timestamptz not null, "question_id" varchar(255) not null, "answer" varchar(255) not null, "questionId" uuid not null, constraint "survey_answer_pkey" primary key ("id"));`);

    this.addSql(`create table "survey_question_answer" ("id" uuid not null default gen_random_uuid(), "created_at" timestamptz not null, "updated_at" timestamptz not null, "user_id" varchar(255) not null, "ai_result" varchar(255) not null, "question_id" uuid not null, "answer_id" uuid not null, constraint "survey_question_answer_pkey" primary key ("id"));`);
