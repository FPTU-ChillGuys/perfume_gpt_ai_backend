...
export class LogTool {
  ...
  private createActivityReportTool(
    period: PeriodEnum,
    periodName: string,
    descriptionSuffix: string
  ): Tool {
    const description =
      `Get a text activity report for a specific user within a ${periodName}. ` +
    ...
    return tool({
      ...
      execute: async (input) => {
        this.logger.log(`[getUserActivityReportPer${periodName}] called for userId: ${input.userId}`);
  ...
  getUserActivityReportPerWeek: Tool = this.createActivityReportTool(
    PeriodEnum.WEEKLY,
    'week',
    'Best for trend analysis.'
  );

  getUserActivityReportPerMonth: Tool = this.createActivityReportTool(
    PeriodEnum.MONTHLY,
    'month',
    'Best for trend analysis.'
  );

  getUserActivityReportPerYear: Tool = this.createActivityReportTool(
    PeriodEnum.YEARLY,
    'year',
    'Best for trend and long-term analysis.'
  );
  ...
  getUserLogSummaryByUserId: Tool = tool({
    description:
      'Get the rolling behavior summary of a specific user. ' +
      'Returns a pre-built text summary of their keywords, intents, active hours, and event types. ' +
      'Use this for personalized recommendations and chatbot context.',
    inputSchema: z.object({
      userId: z.string().describe('The ID of the user')
