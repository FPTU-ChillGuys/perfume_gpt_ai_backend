...
export class ReviewController {
  ...
  async getReviews(
    @Query() request: GetPagedReviewRequest
  ...
  @ApiOperation({
    summary: 'Kiểm tra trạng thái hoàn thành của job tóm tắt đánh giá'
  })
  @ApiBaseResponse(Object) // Trả về dynamic object
  @ApiQuery({
    name: 'variantId',
    description: 'ID của variant sản phẩm đã dùng tạo job',
    required: true
  })
  ...
  @ApiParam({ name: 'variantId', description: 'ID của variant sản phẩm' })
  async getReviewSummaryByVariantId(
    @Param('variantId') variantId: string
  ...
  async addReviewLog(
    ...
    return await this.reviewService.addReviewLog(
      request.type,
      request.variantId,
      request.reviewLog
    );
