...
export const queryItemSchema = z.object({
  purpose: z
    .enum(['search', 'function', 'profile'])
    .describe(
      'Purpose of this query: search=keyword product search, function=call backend function, profile=fetch user profile preferences then search'
    ),
  logic: z
    .array(z.union([z.string(), z.array(z.string())]))
    .nullable()
    .describe(
      'CNF logic for this specific query purpose only. Keep each query focused on ONE intent.'
    ),
  productNames: z
    .array(z.string())
    .nullable()
  ...
  budget: budgetSchema,
  functionCall: functionCallSchema.describe(
    'Only used when purpose="function". Specifies which backend function to call.'
  ),
  profileHint: z
    .string()
    .nullable()
    .describe(
      'Only used when purpose="profile". Hint for the system on what to extract from user profile/order history (e.g. "sở thích mùi hương", "phong cách cá nhân").'
    )
});
...
export const analysisOutputSchema = z.object({
  intent: z
    ...
    .describe(
      'The core intent of the user message. Use "Recommend" for advice/suggestions requesting personalized picks, "Task" for actions.'
    ),
  ...
  queries: z
    ...
    .nullable()
    .describe(
      'Array of decomposed sub-queries. Each query has ONE purpose and runs independently. Results are merged. If user asks multiple things (e.g. "bestseller + my taste + autumn"), split into separate queries. If null/empty, the system falls back to legacy root-level fields below.'
    ),
  // ====== Legacy root-level fields (backward-compatible, used as fallback) ======
  logic: z
    .array(z.union([z.string(), z.array(z.string())]))
    .describe(
      '[LEGACY] Conjunctive Normal Form (CNF) logic for product attributes. Prefer using queries[] instead.'
    ),
  productNames: z
    .array(z.string())
    .nullable()
  ...
  budget: budgetSchema,
  functionCall: functionCallSchema.describe(
    '[LEGACY] Instructs the system to use a specific backend function. Prefer using queries[] with purpose="function" instead.'
  ),
  pagination: z
    .object({
      pageNumber: z.number(),
    ...
    .nullable(),
  originalRequestVietnamese: z
    .string()
    .describe("The user's original message in Vietnamese."),
  normalizationMetadata: z
    ...
    .nullable()
    .describe('Log of keyword normalization/correction process.'),
  explanation: z
    .string()
    .describe(
