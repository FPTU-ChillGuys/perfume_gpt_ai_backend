...
import {
  SlowStockCandidate,
  SlowStockVariantCandidate
} from 'src/application/dtos/response/slow-stock/slow-stock-analytics.types';
import {
  SLOW_STOCK_CONFIG,
  RESTOCK_CONFIG
} from 'src/application/constant/inventory.constant';
...
export class SlowStockService {
  ...
  async getSlowStockCandidates(): Promise<
    BaseResponseAPI<SlowStockCandidate[]>
  > {
    return this.err.wrap(async () => {
      ...
      const variantsWithStock = this.filterSlowMovingVariants(
        analyticsResult.payload,
        stockMap
      );
      const candidates = this.groupVariantsByProduct(
        variantsWithStock,
        stockMap
      );
  ...
  private buildStockMap(
    stocks: Awaited<
      ReturnType<SlowStockRepository['findAllStocksWithRelations']>
    >
  ): Map<
    string,
    {
  ...
  private filterSlowMovingVariants(
    ...
    for (const variant of variants) {
      ...
      if (variant.status === 'Inactive' || variant.status === 'Discontinue')
        continue;

      const avgDaily = variant.averageDailySales ?? 0;
      const forecastDemand =
        SLOW_STOCK_CONFIG.FORECAST_RECENT_WEIGHT * avgDaily +
        SLOW_STOCK_CONFIG.FORECAST_SHORT_TERM_WEIGHT *
          ((variant.salesMetrics?.last7DaysSales ?? 0) / 7);
      const daysOfSupply =
        stock.totalQuantity /
        Math.max(forecastDemand, SLOW_STOCK_CONFIG.MIN_FORECAST_DEMAND);
  ...
  private computeCandidateAggregates(
    candidates: SlowStockCandidate[]
  ): SlowStockCandidate[] {
    candidates.forEach((item) => {
      const avgLast30 =
        item.last30DaysSales / Math.max(item.variantCount, 1) / 4;
      ...
      if (avgLast7 < avgLast30 * RESTOCK_CONFIG.TREND_DECLINING_THRESHOLD) {
        ...
        avgLast7 >
        avgLast30 * RESTOCK_CONFIG.TREND_INCREASING_THRESHOLD
      ) {
  ...
  private sortAndLimitCandidates(
    ...
    return candidates
      .sort((a, b) => {
        const aForecast =
          SLOW_STOCK_CONFIG.FORECAST_RECENT_WEIGHT *
            (a.averageDailySales / a.variantCount) +
          SLOW_STOCK_CONFIG.FORECAST_SHORT_TERM_WEIGHT *
            (a.last7DaysSales / 7 / a.variantCount);
        const bForecast =
          SLOW_STOCK_CONFIG.FORECAST_RECENT_WEIGHT *
            (b.averageDailySales / b.variantCount) +
          SLOW_STOCK_CONFIG.FORECAST_SHORT_TERM_WEIGHT *
            (b.last7DaysSales / 7 / b.variantCount);
