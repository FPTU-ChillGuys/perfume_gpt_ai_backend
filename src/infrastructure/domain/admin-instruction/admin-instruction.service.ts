...
export class AdminInstructionService {
  ...
  > {
    return this.err.wrap(async () => {
      ...
      const response = instructions.map(
        (i) => AdminInstructionResponse.fromEntity(i)!
      );
  ...
  async getInstructionById(
    ...
    return this.err.wrap(async () => {
      const instruction = await this.unitOfWork.AdminInstructionRepo.findOne({
        id
      });
      if (!instruction) {
  ...
  async getInstructionsByType(
    ...
    return this.err.wrap(async () => {
      const instructions =
        await this.unitOfWork.AdminInstructionRepo.findByType(type);
      const response = instructions.map(
        (i) => AdminInstructionResponse.fromEntity(i)!
      );
      return { success: true, data: response };
