...
export class Migration20260330143000_RepairEventLogConstraints extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`update "event_log" set "event_type" = 'survey' where "event_type" = 'quiz';`);
    this.addSql(`update "event_log" set "entity_type" = 'survey' where "entity_type" = 'quiz';`);
  ...
  override async down(): Promise<void> {
    ...
    this.addSql(`alter table "event_log" add constraint "event_log_event_type_check" check ("event_type" in ('message', 'search'));`);
    this.addSql(`alter table "event_log" add constraint "event_log_entity_type_check" check ("entity_type" in ('conversation', 'search'));`);
  }
