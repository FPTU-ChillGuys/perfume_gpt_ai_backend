...
- Thêm dữ liệu mặc định cho **8 domain AI** (review, order, inventory, trend, recommendation, **repurchase**, log, conversation)
...
| `recommendation` | Hướng dẫn gợi ý sản phẩm mới                | `POST /recommendation/recommend/*`       |
| `repurchase`     | Hướng dẫn gợi ý mua lại sản phẩm cũ        | `POST /recommendation/repurchase/*`      |
...
Controller phức tạp nhất — quản lý chatbot AI tư vấn nước hoa. Gồm **9 phiên bản chat (V1–V9)** và CRUD conversation.
...
| `POST` | `/conversation/chat/v8` | Chat V8 — không lấy log, chỉ dùng admin instruction (userId từ body/token/uuid) | 🌐 Public (JWT tùy chọn) |
| `POST` | `/conversation/chat/v9` | Chat V9 — **SSE streaming** (`text/event-stream`), không lấy log | 🌐 Public (JWT tùy chọn) |
...
>
> **V7** sử dụng `ConversationRequestDtoV2` và `buildCombinedPromptV4` — chỉ cần log service + admin instruction. **V8** dùng `buildCombinedPromptV5` (không lấy log). **V9** giống V8 nhưng trả về SSE stream (`pipeUIMessageStreamToResponse`).
...
> **Lưu ý:**
>
> - **V1/V3/V5** dùng log tóm tắt (nhanh). **V2/V4/V6** dùng log chi tiết (đầy đủ hơn). **V7** dùng prompt builder V4 (nhẹ nhất). **V8/V9** không lấy log, chỉ dùng admin instruction.
> - **V5/V6/V7** dùng Bull queue — lưu conversation trong background, tránh timeout.
> - `userId` được lấy từ JWT token. V7/V9 tự tạo UUID nếu không có token.
> - CRUD endpoints (GET conversation) yêu cầu `@Role('admin')`.
> - Conversation được tự động lưu vào PostgreSQL DB.
> - **V9** trả về SSE stream (`Content-Type: text/event-stream`). Không lưu conversation sau khi stream.
...
| `GET` | `/products/all/with-variants` | \[TEST\] Danh sách sản phẩm kèm toàn bộ variants (phân trang) | 🌐 Public |
| `GET` | `/products/search/with-variants?searchText=` | Semantic search kèm toàn bộ variants | 🌐 Public |
| `GET` | `/products/:id/with-variants` | \[TEST\] Chi tiết một sản phẩm kèm toàn bộ variants | 🌐 Public |
...
**Route:** `/profile` | **Auth:** 🔒 JWT + Role `admin` hoặc `user` | **Tag Swagger:** `Profile`

Lấy thông tin profile của người dùng hiện tại từ backend .NET thông qua JWT token.

| Method | Endpoint | Mô tả | Auth |
|--------|----------|-------|------|
| `GET` | `/profile/me` | Lấy thông tin profile từ token | 🔒 admin/user |
| `GET` | `/profile/report` | Tạo báo cáo profile dưới dạng text | 🔒 admin/user |
...
> **Lưu ý:**
>
> - Yêu cầu role `admin` **hoặc** `user` (class-level `@Role(['admin', 'user'])`).
...
**Route:** `/reviews` | **Auth:** 🔒 JWT + Role `admin` | **Tag Swagger:** `Reviews`
...
| `GET` | `/reviews/logs/variant/:variantId` | Lấy review logs theo variant ID | 🔒 admin |
| `GET` | `/reviews/logs/:id` | Lấy review log theo ID | 🔒 admin |
...
| Method | Endpoint | Mô tả | Auth |
|--------|----------|-------|------|
| `GET` | `/logs/report/activity/all` | Lấy báo cáo tất cả log hoạt động (raw text) | 🌐 Public |
...
> **Lưu ý:**
>
> - Class-level `@Role('admin')`, ngoại trừ `report/activity/all` có `@Public()` (kèm `@CacheTTL(1)`).
...
**Route:** `/trends` | **Auth:** Hỗn hợp (`admin`/`user` và Public) | **Tag Swagger:** `Trends`

Dự đoán xu hướng nước hoa dựa trên tổng hợp log hoạt động của tất cả người dùng.

| Method | Endpoint | Mô tả | Auth |
|--------|----------|-------|------|
| `GET` | `/trends/summary` | Dự đoán xu hướng bằng AI (text) | 🔒 admin/user |
| `GET` | `/trends/product` | Lấy sản phẩm trending từ phân tích xu hướng (không cache) | 🌐 Public |
| `GET` | `/trends/product/caching` | Lấy sản phẩm trending (cache 24h) | 🌐 Public |
| `GET` | `/trends/summary/structured` | Dự đoán xu hướng AI có cấu trúc (JSON + metadata) | 🔒 admin/user |
...
> **Lưu ý:**
>
> - Class-level `@Role(['admin', 'user'])` — tất cả endpoint yêu cầu đăng nhập, ngoại trừ `/product` và `/product/caching` có `@Public()`.
...
> - Endpoint `/product` và `/product/caching` sử dụng `CacheInterceptor` + `@CacheTTL()` cho response caching.
