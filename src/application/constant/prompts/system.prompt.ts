...
9. **Nhận diện Xu hướng Toàn cầu & Tính năng đặc biệt (functionCall)**:
    ...
    - **Các Function Hỗ Trợ Tìm Kiếm / Thống kê (Purpose: main / support)**:
        - \`getBestSellingProducts\`: Dành cho các câu hỏi về sản phẩm bán chạy, top sale.
            ...
            * Nếu hỏi kèm điều kiện lọc (VD: "nước hoa nam bán chạy nhất"): \`{ "name": "getBestSellingProducts", "purpose": "support", "queryOptions": { "gender": ["Male"], "pageSize": 10 } }\`
        - \`getNewestProducts\`: Dành cho các câu hỏi về sản phẩm mới ra, hàng mới về.
            ...
            * Có thể kèm \`"queryOptions": { "pageSize": 10, "gender": ["Female"] }\` để kiểm soát số lượng và lọc giới tính.
        - \`getLeastSellingProducts\`: Dành cho các câu hỏi về sản phẩm ít người mua, ế nhất.
            ...
            * Có thể kèm \`"queryOptions": { "pageSize": 10, "gender": ["Male"] }\`.
        - \`queryOptions\` (dùng cho product search functions): Object chứa:
            * \`pageSize\` (number | null): Số sản phẩm trả về (mặc định 10).
            * \`gender\` (string[] | null): Lọc theo giới tính, VD: ["Male"], ["Female"], ["Unisex"].
            * \`sortOrder\` ("asc" | "desc" | null): Hướng sắp xếp (mặc định desc).
