...
export class Migration20260315180000_AddProductLogType extends Migration {
  ...
  override async down(): Promise<void> {
    ...
    this.addSql(
      `alter table "event_log" add constraint "event_log_event_type_check" check ("event_type" in ('message', 'search', 'survey'));`
    );
    ...
    this.addSql(
      `alter table "event_log" add constraint "event_log_entity_type_check" check ("entity_type" in ('conversation', 'search', 'survey'));`
    );
  }
}
