...
export function mergeSurveyQueryResults(
  ...
  const scoreMap = new Map<
    string,
    { product: any; score: number; sources: string[] }
  >();
  ...
  if (validResults.length === 0) {
    console.log(
      `[SurveyMerge] No queries with products found. Returning empty array.`
    );
    return [];
  }

  console.log(
    `[SurveyMerge] Filtering: ${results.length} total queries -> ${validResults.length} queries with products`
  );
  ...
  console.log(
    `[SurveyMerge] Merged ${topProducts.length} unique products from ${validResults.length} queries with results`
  );
  ...
  if (topProducts.length > 0) {
    const topProductNames = topProducts
      ...
      .join(', ');
    console.log(
      `[SurveyMerge] Top products: [${topProductNames}]${topProducts.length > 3 ? '...' : ''}`
    );
  }
...
export function buildSurveyContextForAI(
  ...
  const questionsSection = quesAnses
    .map(
      (qa, index) => `Câu ${index + 1}: ${qa.question}\nTrả lời: ${qa.answer}`
    )
    .join('\n\n');
...
export function getMergeStatistics(results: SurveyQueryResult[]): {
  ...
  return {
    ...
    queriesWithResults: results.filter(
      (r) => r.products && r.products.length > 0
    ).length,
